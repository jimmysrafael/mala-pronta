...
async function searchAttractions({ destination }) {
  ...
  try {
    const cached = await db.one('SELECT resultado_json, created_at FROM attraction_cache WHERE city_name = ?', [city]);
    if (cached) {
  ...
  try {
    ...
    await db.run(
      'INSERT INTO attraction_cache (city_name, resultado_json, created_at) VALUES (?, ?, NOW()) ON CONFLICT (city_name) DO UPDATE SET resultado_json = EXCLUDED.resultado_json, created_at = NOW()',
      [city, JSON.stringify(attractions)]
    );
